        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Approved Cases</h3>
                </div>
                <!-- /.box-header -->
                ...
                <!--<div class="box-body table-responsive no-padding">-->
                    <table id="users_list" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        @foreach($pm_case_record as $case_record)
                            <tr>
                                <td style="display:none">YYYY/mm/dd</td>
                                <td>{{ $case_record->team }}-{{ $case_record->district }}-{{ $case_record->id }}</td>
                                <td>{{ $case_record->fmccrsid }}</td>
                                <td>{{ $case_record->zmccrsid }}</td>
                                <td>{{ $case_record->nsmccrsid }}</td>
                                <td>{{ $case_record->team }}</td>
                                <td>{{ $case_record->station }}</td>
                                <td>{{ $case_record->dr_name }}</td>
                                <td>Approved</td>
                                <td><a href="{{route('pm.view_case',$case_record->id)}}">View Case</a> | <a href="{{route('pm.generate-pdf',$case_record->id)}}">PDF</a></td>
                            </tr>
                        @endforeach
                        </tbody>
                        <!-- <tfoot>
